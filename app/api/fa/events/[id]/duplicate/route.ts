...
import { duplicateEventRow } from "@fa/_lib/events.server";

export const dynamic = "force-dynamic";

/**
 * POST /api/fa/events/[id]/duplicate
 * Body: { name, startDate, endDate, invitationOpenDate, invitationCloseDate, notes? }
 *
 * Clones the source event's session + quota layout into a new draft event with
 * the supplied name/dates. Auth: any signed-in user (the marketing page already
 * gates this surface to MKT).
 */
export async function POST(
  ...
  try {
    ...
    console.error("[api/fa/events/duplicate] failed:", err);
