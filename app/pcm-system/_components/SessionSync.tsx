 ...
 *    SUPER_ADMIN / ADMIN / ACADEMY → "back-office" roles. Default to the
 *                           PCM Academy view (u-mkt) on first arrival but
 *                           DO NOT override a manual pick from
 *                           /pcm-system/login, so all of them can use the
 *                           picker to switch between Academy and any
 *                           Branch Manager view.
 *    BRANCH_MANAGER       → forced to PCM BM for the branch whose name
...
export function SessionSync() {
  ...
  useEffect(() => {
    ...
    if (isBackOfficeRole(role)) {
      ...
      // /pcm-system/login can switch them into any Branch Manager view.
    ...
    if (role === "BRANCH_MANAGER" && branchName) {
      ...
      if (required && currentUserId !== required) {
        ...
        // they don't get stuck on a PCM page they can't read. Also surface
    ...
    // Unrecognised role → no PCM access.
    if (currentUserId !== null) logout();
